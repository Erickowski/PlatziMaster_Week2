...
void merge (int left[], int nextLeft, int right[], int nextRight, int s[]) {
	int i = 0, j = 0, k = 0;
	while (i < nextLeft && j < nextRight) {
		if (left[i] <= right[j]) {
			s[k] = left[i];
			i++;
		} else {
...
void mergeSort (int s[], int n) {
	if (n < 2) return;
	int middle = n / 2, left[middle], right[n - middle];
	for (int i = 0; i < middle; i++) {
		left[i] = s[i];
	}
	for (int i = 0; middle < n; i++) {
		right[i] = s[middle];
		middle++;
	}
	int nextLeft = sizeof(left) / sizeof(int);
	int nextRight = sizeof(right) / sizeof(int);
	mergeSort(left, nextLeft);
	mergeSort(right, nextRight);
	merge(left, nextLeft, right, nextRight, s);
}
